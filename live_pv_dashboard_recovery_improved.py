...
st.caption(
    ...
    "Damage Index, maintenance-oriented RUL estimation, and recovery monitoring"
)
...
def show_recovery_status(hi_df):
    """Display post-degradation recovery monitoring status."""
    if hi_df is None or len(hi_df) == 0:
        st.info("Recovery status is not available yet.")
        return

    latest = hi_df.iloc[-1]

    st.subheader("Stable Health / Recovery Status")

    r1, r2, r3, r4 = st.columns(4)

    if "stable_healthy_duration_days" in hi_df.columns and pd.notna(latest.get("stable_healthy_duration_days")):
        r1.metric(
            "Stable Healthy Duration", f"{latest['stable_healthy_duration_days']:.1f} days"
        )
    else:
        r1.metric("Stable Healthy Duration", "N/A")

    r2.metric(
        "Recovery Candidate",
        "Yes" if bool(latest.get("recovery_candidate", False)) else "No"
    )

    r3.metric(
        "Recovery Confirmed",
        "Yes" if bool(latest.get("recovery_confirmed", False)) else "No"
    )

    r4.metric(
        "New Baseline Candidate",
        "Yes" if bool(latest.get("new_baseline_candidate", False)) else "No"
    )

    if bool(latest.get("new_baseline_candidate", False)):
        st.success(
            "Sustained recovery has been observed for at least 30 days. "
            "A new post-recovery baseline is recommended."
        )
    elif bool(latest.get("recovery_confirmed", False)):
        st.success(
            "Sustained recovery is confirmed. Damage index is partially reset/stabilized, "
            "but the system does not assume the PV has returned to day-one condition."
        )
    elif bool(latest.get("recovery_candidate", False)):
        st.info(
            "Recovery candidate detected. Continue monitoring before confirming post-recovery baseline."
        )
    else:
        st.caption(
            "Stable Healthy Duration measures how long the PV system remains healthy "
            "without persistent degradation. The counter resets only when a new "
            "persistent degradation cycle is detected."
        )

    if "partial_damage_reset_applied" in hi_df.columns:
        reset_status = "Applied" if bool(latest.get("partial_damage_reset_applied", False)) else "Not Applied"
        st.caption(f"Partial damage reset status: {reset_status}")


def build_recovery_event_log(hi_df):
    if hi_df is None or len(hi_df) == 0:
        return pd.DataFrame()
    ...
    if "recovery_candidate" in hi_df.columns:
        candidate_events = hi_df[hi_df["recovery_candidate"] == True].tail(3)
        for _, row in candidate_events.iterrows():
            events.append({
                "Time": row["window_mid"],
                "Event": "Recovery candidate detected",
                "HI": round(row[hi_col], 3)
            })

    if "recovery_confirmed" in hi_df.columns:
        confirmed_events = hi_df[hi_df["recovery_confirmed"] == True].tail(3)
        for _, row in confirmed_events.iterrows():
            events.append({
                "Time": row["window_mid"],
                "Event": "Sustained recovery confirmed",
                "HI": round(row[hi_col], 3)
            })

    if "new_baseline_candidate" in hi_df.columns:
        baseline_events = hi_df[hi_df["new_baseline_candidate"] == True].tail(3)
        for _, row in baseline_events.iterrows():
            events.append({
                "Time": row["window_mid"],
                "Event": "New post-recovery baseline candidate",
                "HI": round(row[hi_col], 3)
            })
...
if section == "Overview":
...
elif section == "Recovery Monitoring":
    st.header("Recovery Monitoring")
    show_system_health_summary(hi_df, drift_df)
    st.divider()
    show_recovery_status(hi_df)
    st.divider()
    st.subheader("Recovery-Related Events")
    recovery_log = build_recovery_event_log(hi_df)
    if len(recovery_log) > 0:
        st.dataframe(recovery_log, use_container_width=True)
    else:
        st.info("No recovery event detected yet.")
    ...
    combined_logs = []
    if len(event_log) > 0:
        combined_logs.append(event_log)
    if len(recovery_log) > 0:
        combined_logs.append(recovery_log)

    if len(combined_logs) > 0:
        all_events = pd.concat(combined_logs, ignore_index=True)
        all_events = all_events.drop_duplicates().sort_values("Time").tail(15)
        st.dataframe(all_events, use_container_width=True)
    else:
        st.info("No major monitoring or recovery events detected yet.")
